...
import { 
  ...
  List, ListOrdered, Quote, Link, Lightbulb, Search, PenTool,
  ChevronDown, BookOpen, Target, Clock, History, Eye, RotateCcw,
  HelpCircle, Save
} from 'lucide-react';
...
const sectionContent: Record<string, SectionConfig> = {
  ...
  'intro-problem': {
    ...
    placeholder: 'Clearly articulate the the problem your research addresses...',
...
export function ContentEditor({
  ...
  // Capture selection when the user interacts with the editor
  useEffect(() => {
  ...
  // Insert citation at caret (restoring saved caret if needed)
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (sel2 && sel2.rangeCount > 0 && isInsideEditor(sel2.getRangeAt(0).startContainer)) {
        ...
        if (editorRef.current.lastChild) {
          after.setStartAfter(editorRef.current.lastChild);
          after.collapse(true);
          sel2.addRange(after);
        }
      } else {
  ...
  // Load content & versions on section change
  useEffect(() => {
  ...
  const saveDraft = useCallback((sectionId: string, content: string) => {
    ...
    setTimeout(() => {
      ...
      if (content.trim().length > 10) {
        ...
        setVersions(updated);
        localStorage.setItem(`paper_${paper.id}_section_${sectionId}_versions`, JSON.stringify(updated));
      }
  ...
  const handleContentChange = (e: React.FormEvent<HTMLDivElement>) => {
    ...
    saveTimeoutRef.current = setTimeout(() => saveDraft(activeSection, newContent), 1000);
  };
  ...
  const handleManualSave = useCallback(() => {
    const current = editorRef.current?.textContent || content || '';
    saveDraft(activeSection, current);
  ...
  const handleSuggestCitationsForSelection = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Failed to suggest citations');
    ...
    <div className="h-full min-h-0 flex flex-col">
      ...
      <Dialog open={showRubric} onOpenChange={setShowRubric}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>Best practices and structure for this section</DialogDescription>
          </DialogHeader>
      ...
      <style jsx>{`
        ...
        [contenteditable] blockquote {
          border-left: 4px solid #3b82f6; margin: 1rem 0; padding-left: 1rem; font-style: italic;
        }
        [contenteditable] ul, [contenteditable] ol { margin: 1rem 0; padding-left: 2rem; }
        [contenteditable] li { margin: 0.5rem 0; }
        [contenteditable] h1, [contenteditable] h2, [contenteditable] h3 { font-weight: bold; margin: 1.5rem 0 1rem 0; }
        [contenteditable] h1 { font-size: 1.5rem; }
        [contenteditable] h2 { font-size: 1.25rem; }
        [contenteditable] h3 { font-size: 1.125rem; }
        [contenteditable] p { margin: 1rem 0; }
      `}</style>
    </div>
  );
}
